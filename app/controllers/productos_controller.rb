class ProductosController < ApplicationController
  before_action :set_producto, only: %i[ show edit update destroy new_movement create_movement ]
  ...
  def new_movement
    @movement = @producto.movements.new
  end

  def create_movement
     @movement = @producto.movements.new(movement_params)
    if @movement.save
      redirect_to @producto, notice: "Movimiento registrado."
    else
  ...
  private
    ...
    def producto_params
      params.expect(producto: [ :nombre, :referencia, :descripcion ])
    end

    def movement_params
      params.require(:movement).permit(:movement_kind, :quantity, :comment)
    end
end